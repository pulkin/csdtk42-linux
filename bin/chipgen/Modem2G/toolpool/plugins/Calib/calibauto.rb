    ...
    def bbMeasureRxPower(band, arfcn, expPow)
        ...
        begin
			...
			if( nbMeas < NB_MEAS_ILOSS)
				...
				raise "Error : iloss too high (+ %d not in [%d, %d])" % [defaultIloss, ILOSS_MIN, ILOSS_MAX]
			end
			
    		# iloss is saved as 4 bits on target and is regarded as an unsigned value
			ilossOutOfRange = false;
		    if( (iloss+defaultIloss) < ILOSS_MIN || (iloss+defaultIloss) > ILOSS_MAX )
		        ilossOutOfRange = true;
			end
		    
		end while( ilossOutOfRange || measure != lastMeasure || measure != lastLastMeasure )
